...
const mockValueGenerator_1 = require("./mockValueGenerator");
...
const createNameConverter = (convention, transformUnderscore) => (value, prefix = '') => {
    ...
    return `${prefix}${convertName(value, (0, plugin_helpers_1.resolveExternalModuleAndFn)(convention), transformUnderscore)}`;
};
...
const getCasualCustomValue = (generatorDefinition, opts) => {
    ...
    const embeddedGenerator = casual_1.default[generatorDefinition.generator];
...
const getNamedType = (opts) => {
    ...
    const mockValueGenerator = (0, mockValueGenerator_1.setupMockValueGenerator)({
...
const plugin = (schema, documents, config) => {
    ...
    const printedSchema = (0, utils_1.printSchemaWithDirectives)(schema); // Returns a string representation of the schema
    ...
    const mockFns = definitions
        ...
        .map((mockFn) => mockFn())
        .join('\n');
    const functionTokens = (0, mockValueGenerator_1.setupFunctionTokens)(generateLibrary, generatorLocale);
    let mockFile = '';
    if (dynamicValues)
